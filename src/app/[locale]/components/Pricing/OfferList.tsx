// components/Pricing/ServerCalculator.tsx
"use client";
...
const ServerCalculator = () => {
  ...
  const getDurationLabel = (count: number) => {
    if (isMonthly) {
      return count === 1 ? t('month') : t('monthsPlural');
    } else {
      return count === 1 ? t('year') : t('yearsPlural');
    }
  };
        ...
        <div className="mx-auto max-w-[800px]">
          ...
          <div className="rounded-lg bg-white p-8 shadow-lg dark:bg-gray-dark">
            ...
            <div className="mb-8">
              <div className="mb-3 flex items-center justify-between">
                <label className="text-base font-semibold text-dark dark:text-white">
                  {t('duration')} ({isMonthly ? t('months') : t('years')})
                </label>
            ...
            <div className="border-t border-body-color border-opacity-10 pt-8">
              <div className="mb-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-body-color">{t('ram')} ({ram} GB × ${pricing.ram})</span>
                  <span className="font-medium text-dark dark:text-white">
                    ${ram * pricing.ram}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-body-color">{t('cpu')} ({cpu} vCPU × ${pricing.cpu})</span>
                  <span className="font-medium text-dark dark:text-white">
                    ${cpu * pricing.cpu}
                  </span>
                </div>
                {gpu > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-body-color">{t('gpu')} ({gpu} GPU × ${pricing.gpu})</span>
                    <span className="font-medium text-dark dark:text-white">
                      ${gpu * pricing.gpu}
                    </span>
                  </div>
                ...
                {duration > 1 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-body-color">
                      {t('duration')} ({duration} {getDurationLabel(duration)})
                    </span>
              ...
              <div className="mb-6 flex items-center justify-between border-t border-body-color border-opacity-10 pt-4">
                ...
                <div className="text-right">
                  ...
                  <span className="ml-2 text-base font-medium text-body-color">
                    / {duration === 1 ? getDurationLabel(1) : `${duration} ${getDurationLabel(duration)}`}
                  </span>
                </div>
              </div>
          ...
          <div className="mt-12 grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-gray-dark">
              ...
              <div className="text-sm text-body-color">{t('support')}</div>
            </div>
            <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-gray-dark">
              <div className="mb-3 text-2xl font-bold text-primary">99.9%</div>
              <div className="text-sm text-body-color">{t('sla')}</div>
            </div>
            <div className="rounded-lg bg-white p-6 text-center shadow-md dark:bg-gray-dark">
              <div className="mb-3 text-2xl font-bold text-primary">Instant</div>
              <div className="text-sm text-body-color">{t('scaling')}</div>
            </div>
          </div>
        </div>
      </div>
